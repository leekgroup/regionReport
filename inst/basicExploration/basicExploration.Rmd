...
This report is meant to help explore the results of the `derfinder` `r citep(bib[['derfinder']])` package and was generated using the `regionReport` `r citep(bib[['regionReport']])` package. While the report is rich, it is meant to just start the exploration of the results and exemplify some of the code used to do so. You will most likely need a more in-depth analysis for your specific data set.
