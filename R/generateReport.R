...
#'
#' This function generates a HTML report exploring the basic results from derfinder (https://github.com/lcolladotor/derfinder). The HTML report itself is generated using knitrBoostrap (https://github.com/jimhester/knitrBootstrap) which uses knitr (http://yihui.name/knitr/) behind the scenes. It works best after using \link[derfinder]{mergeResults}.
#' 
...
#' @param makeBestClusters If \code{TRUE}, \link[derfinder]{plotCluster} is used on the \code{nBestClusters} regions by area. Note that these plots take some time to make.
