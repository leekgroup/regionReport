...
This report is meant to help explore the results of the `derfinder` `r citep(bib[["derfinder"]])` package and was generated using the `derfinderReport` `r citep(bib[["derfinderReport"]])` package. While the report is rich, it is meant to just start the exploration of the results and exemplify some of the code used to do so. You will most likely need a more in-depth analysis for your specific data set.
...
xrange <- range(log10(regions.df$area[finite.area]))
if(inf.area > 0) {
...
p3b <- ggplot(regions.df[intersect(idx.sig, finite.area), ], aes(x=log10(area), colour=seqnames)) + geom_line(stat="density") + labs(title="Density of region areas (significant only)") + xlab("Region area (log10)") + scale_colour_discrete(limits=chrs) + xlim(xrange) + theme(legend.title=element_blank())
...
for(j in grep("log2FoldChange", colnames(values(fullRegions)))) {
	...
	ma <- data.frame(mean=ma.mean, log2FoldChange=values(fullRegions)[, j])
	ma2 <- ma[is.finite(ma$log2FoldChange), ]
	fold.mean <- data.frame(foldMean=mean(ma2$log2FoldChange, na.rm=TRUE))
	
	p.ma <- ggplot(ma, aes(x=log2(mean), y=log2FoldChange)) + geom_point(size=1.5, alpha=1/5) + ylab("Fold Change (log2)\nRed dashed line at mean; blue line is loess fit") + xlab(paste("Mean coverage (log2) using only groups", groups[1], "and", groups[2])) + labs(title=paste("MA style plot:", groups[1], "vs ", groups[2])) + geom_hline(aes(yintercept=foldMean), data=fold.mean, colour="#990000", linetype="dashed") + geom_smooth(aes(y=log2FoldChange, x=log2(mean)), data=subset(ma2, mean > 0), method=loess)
	print(p.ma)
}
```
...
Below is an interactive table with the top `r min(nrow(regions.df), 2000)` regions (out of `r nrow(regions.df)`) as ranked by area. This table was generated using `rCharts` `r citep(bib[["rCharts"]])`. Inf and -Inf are shown as 1e100 and -1e100 respectively.
...
The following plots were made using `ggbio` `r citep(bib[["ggbio"]])` which in turn uses `ggplot2` `r citep(bib[["ggplot2"]])`. For more details check `plotRegion` in `derfinder` `r citep(bib[["derfinder"]])`. 
...
Note that in `derfinder` `r citep(bib[["derfinder"]])` the re-sampling of the samples is done without replacement. This is done in an effort to avoid singular model matrices. While the sample balance is the same across the permutations, what changes are the adjusted variables (including the column medians).
...
This report is part of `derfinder` `r citep(bib[["derfinder"]])` and was generated using `knitr` `r citep(bib[["knitr"]])` and styled via `knitrBootstrap` `r citep(bib[["knitrBootstrap"]])`.
