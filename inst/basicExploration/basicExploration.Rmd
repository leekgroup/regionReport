...
This report is meant to help explore the results of the `derfinder` `r citep(bib[['colladotorres2014derfinder']])` package and was generated using `regionReport` `r citep(bib[['colladotorres2014regionreport']])` package. While the report is rich, it is meant to just start the exploration of the results and exemplify some of the code used to do so. You will most likely need a more in-depth analysis for your specific data set.

Most plots were made with using `ggplot2` `r citep(bib[['wickham2009ggplot2']])`.
...
seqlevelsStyle(fullRegions) <- chrsStyle
...
if(nBestRegions > 0) {
    if(packageVersion('derfinder') >= '0.0.60') {
        regionCoverage <- getRegionCoverage(fullCov = fullCov, 
            ...
            chrsStyle = chrsStyle, verbose = FALSE)
    } else {
        regionCoverage <- getRegionCoverage(fullCov = fullCov, 
            regions = fullRegions[seq_len(nBestRegions)],
            verbose = FALSE)
...
print(xtable(qtable, digits=4, align=rep('c', 3)), type='html',
...
The following plots were made using `ggbio` `r citep(bib[['yin2012ggbio']])` which in turn uses `ggplot2` `r citep(bib[['wickham2009ggplot2']])`. For more details check `plotOverview` in `derfinder` `r citep(bib[['colladotorres2014derfinder']])`.
...
Below is an interactive table with the top `r min(nrow(regions.df), 2000)` regions (out of `r nrow(regions.df)`) as ranked by area. This table was generated using `rCharts` `r citep(bib[['vaidyanathan2013rcharts']])`. Inf and -Inf are shown as 1e100 and -1e100 respectively.
...
t1 <- dTable(d, sPaginationType=  'full_numbers', iDisplayLength=10,
    sScrollX='100%')
...
The following plots were made using `ggbio` `r citep(bib[['yin2012ggbio']])` which in turn uses `ggplot2` `r citep(bib[['wickham2009ggplot2']])`. For more details check `plotCluster()` in `derfinder` `r citep(bib[['colladotorres2014derfinder']])`. 
...
t2 <- dTable(d2, sPaginationType = 'full_numbers', iDisplayLength=10, 
    sScrollX='100%')
...
This report was created with `regionReport` `r citep(bib[['colladotorres2014regionreport']])` using `knitrBootstrap` `r citep(bib[['hester2014knitrbootstrap']])`
to format the html while `knitr` `r citep(bib[['xie2014knitr']])` and `rmarkdown` `r citep(bib[['allaire2014rmarkdown']])` were running behind the scenes.
